...
import type { ProviderGovernanceGate, ProviderRouteGovernanceRequest } from "../ai-provider-governance/models";
...
export class AiGatewayService {
  constructor(private readonly repository:AiGatewayRepository,private readonly ids:UuidV7,private readonly providers:ReadonlyMap<string,AiProviderPort>,private readonly observations:AiGatewayObservationPort,private readonly now:()=>number=Date.now,private readonly governance?:ProviderGovernanceGate){}
  ...
  async invokeProvider(routes:readonly AiRoute[],request:Parameters<AiProviderPort["invoke"]>[0],governanceRequest?:Omit<ProviderRouteGovernanceRequest,"providerKey"|"modelKey"|"modelVersion">):Promise<{result:AiProviderResult;route:AiRoute}>{for(const route of this.selectRoute(routes)){const external=route.providerKey!=="deterministic_local_adapter";if(external&&(!this.governance||!governanceRequest))continue;if(this.governance&&governanceRequest)await this.governance.authorize({...governanceRequest,...route});const provider=this.providers.get(route.providerKey);if(!provider?.enabled)continue;try{return{result:await provider.invoke(request),route};}catch{}}throw new AiGatewayError("AI_PROVIDER_FAILED");}
  ...
  async execute(input: PrepareAiRequestInput): Promise<AiExecutionResult> {
    ...
    const invoked = shortcut
      ...
      : await this.invokeProvider(policy.routes, { taskKey: input.taskKey, taskVersion: 1, input: input.input, locale: input.locale, maxOutputUnits: input.requestedOutputUnits, timeoutMs: policy.maxLatencyMs }, { taskKey: input.taskKey, taskVersion: 1, environment: "production", tenantId: input.context.tenantId, applicationId: input.context.applicationId, sensitivity: input.taskKey === "diagnostics.safe_summary" ? "restricted" : "internal", inputUnits: canonicalJson(input.input).length, outputUnits: input.requestedOutputUnits, estimatedCostMicros: input.requestedCostMicros, interactive: true });
