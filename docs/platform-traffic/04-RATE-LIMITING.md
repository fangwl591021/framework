...
The Local adapter makes decision and increment one synchronous state transition, so concurrent promises produce exactly the configured winners. Burst is included once, counters never wrap or saturate silently, and overflow fails closed. This is deterministic single-isolate evidence only, not a cross-Worker Production guarantee.
