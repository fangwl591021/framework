...
- 初版使用單一 D1 `batch()`。owner＋generation CAS先把同一 Result Reference暫存為Completed；Projection UPDATE再驗證該 Completed generation、healthy status、expected version與non-negative balance；`trg_point_transactions_projection_guard` 在 Ledger Insert 前比對同 Tenant／Account、healthy status、Version、Watermark／Transaction ID與Resulting Balance。Projection UPDATE為零筆或任一值不符時，Trigger以 `RAISE(ABORT, 'point_projection_guard_mismatch')` 產生 constraint error，使 Idempotency、Projection與Ledger整批rollback；禁止依賴跨 statement affected-row 傳遞、commit後檢查或補償。
...
- Single Full Reverse 由 `trg_point_transactions_reverse_guard` 在 Ledger Insert 前驗證 Original 不是 Reverse、同 Tenant／Account且 signed amount精確相反；不符時以 `RAISE(ABORT, 'point_reverse_guard_mismatch')` 拒絕。`uq_point_transactions_single_full_reverse` 另外阻止第二筆 Full Reverse。
...
## DB Assertion Negative Evidence

下列為 Gate 3 最終審查必須取得的負向證據；本 PR 只定義預期，不執行 SQL 或測試，因此目前全部為 **Not Executed／Not Verified**。

| Case | Forced Mismatch | Expected DB Evidence | Atomic Result |
| --- | --- | --- | --- |
| A01 | Projection UPDATE 為 0 row，仍嘗試 Insert Ledger | `point_projection_guard_mismatch` constraint error | Ledger 不建立；同一 `batch()` 全部 rollback |
| A02 | Projection Version != Ledger `projection_version` | `point_projection_guard_mismatch` constraint error | Ledger 不建立；同一 `batch()` 全部 rollback |
| A03 | Projection Watermark != Ledger `id` | `point_projection_guard_mismatch` constraint error | Ledger 不建立；同一 `batch()` 全部 rollback |
| A04 | Projection Balance != Ledger `resulting_balance` | `point_projection_guard_mismatch` constraint error | Ledger 不建立；同一 `batch()` 全部 rollback |
| A05 | Reverse Amount != `-Original.signed_amount` | `point_reverse_guard_mismatch` constraint error | Reverse 不建立；同一 `batch()` 全部 rollback |
| A06 | Reverse 的 Original 本身為 Reverse | `point_reverse_guard_mismatch` constraint error | Reverse 不建立；同一 `batch()` 全部 rollback |
