...
| Metadata | Value |
| --- | --- |
| `module_id` | business-network-engine |
| `display_name` | Business Network Engine（商業網路引擎） |
...
| `supported_adapters` | None implemented; future provider／payment／presentation adapters remain external |
| `feature_flag_key` | None approved |
| `tenant_scoped` | Yes |
| `shop_scoped` | No |
| `contains_pii` | Pseudonymous buyer／visitor references only; raw provider subject forbidden |
| `audit_required` / `idempotency_required` | Yes / Yes |
| `stable_use_cases` | None |
| `source_assets` | None copied |
