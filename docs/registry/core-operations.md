# Registry Entry: Core Operations

> Candidate · Contract Approved · Not Implemented · Not Verified · Production Use Not Allowed

| Metadata | Value |
| --- | --- |
| `module_id` | `core-operations` |
| `display_name` | Core Operations |
| `description` | Provides Idempotency, Audit, Correlation and Internal Administration contracts |
| `category` | Platform Core Cross-cutting Module |
| `lifecycle_status` | Candidate |
| `contract_status` | Approved |
| `implementation_status` | Not Implemented |
| `verification_status` | Not Verified |
| `deployment_status` | Not Deployed |
| `production_use` | Not Allowed |
| `architecture_approval` | Approved by Tony |
| `current_version` | `0.1.0-draft` |
| `owner` | Unassigned |
| `minimum_core_version` | N/A |
| `maximum_core_version` | N/A |
| `dependencies` | None |
| `optional_dependencies` | None |
| `supported_adapters` | None |
| `feature_flag_key` | None |
| `tenant_scoped` | Mixed; Tenant required for Tenant operations, null only for explicit Platform scope |
| `shop_scoped` | No in Phase 1 |
| `contains_pii` | Minimal confidential references only |
| `audit_required` | Yes; administration and recovery are audited |
| `idempotency_required` | Yes |
| `stable_use_cases` | None |
| `source_assets` | None; no Legacy Runtime source approved |
| `documentation_path` | `docs/runtime-phase-1/00-DECISION-CLOSURE.md` |
| `contract_path` | `docs/runtime-phase-1/04-CORE-OPERATIONS-CONTRACT.md` |
| `deprecation_date` | None |
| `replacement_module` | None |
| `approval_reference` | PR #12／Approved by Tony |
