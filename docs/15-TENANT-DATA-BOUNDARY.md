...
## Conversion and Merge Boundary

- **Conversion Scope**：Registration、Membership Join、Event Registration、Order、Payment、Check-in、Redemption 都必須保留產生它們的 Tenant；Attribution 不得將 Conversion 移至另一 Tenant。
- **Merge Boundary**：Platform User Merge 不會把 Tenant A Membership 覆寫至 Tenant B，也不會將兩個 Tenant 的 Point、Referral、Attribution、Role 或 Consent 合併。
- 同一 Tenant 內的 Membership Conflict 仍需逐 Domain Policy、Historical Correction 與 Audit 處理，不得以更新 `user_id` 取代 Merge Plan。
