...
- Runtime transport route 名稱仍未正式化；可信 Application Context 由未來 composition 建立。
- Configuration schema 驗證目前只提供通用大小、深度與 Secret 規則；各模組專屬 validator 待後續版本。
- Contract 仍為 Proposed；Lifecycle 為 Platform Service Candidate；Production Use Not Allowed。
