...
`main` 已有 Sprint 6 Logical Model；D1 是正式資料的 Source-of-Truth Decision，KV 只能作可重建 Cache。Physical Schema 尚未在 `main` 批准或實作。Sprint 7 Physical D1 Schema Proposal 仍在 [Draft PR #6](https://github.com/fangwl591021/framework/pull/6) 審查，不得視為正式基準。詳見 [Data Architecture](docs/handbook/07-DATA-ARCHITECTURE.md)。
...
Sprint 1～6 已建立文件基礎；Sprint 7 仍在 Draft Review。本 Handbook 只新增理解與導航層，不批准 Runtime、Physical Schema、Migration 或 Deployment。後續候選順序見 [Roadmap](docs/handbook/17-ROADMAP.md)。
