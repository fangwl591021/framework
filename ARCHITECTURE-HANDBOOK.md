...
## 17. QA Governance Foundation

[QA Governance Foundation](docs/qa-governance/README.md) 是所有測試設計、測試計畫批准、執行證據與 QA Gate 的治理入口。閱讀順序為 Governance Status → Standards → Execution Lifecycle → Evidence Review → Approval Gates → Readiness Checklist → Migration Test Plan Mapping。

Migration Test Plan Approval 必須引用這套標準並完成可追溯映射。QA Governance 目前僅為 Proposed；PR #9 仍為 NO-GO，且不代表任何 Test Library 已實作、任何測試已執行或任何 Evidence 已驗證。
