...
export function saveLineIntegrationDraft(snapshot, metadata, now = Date.now()) {
  ...
  return createPlatformSnapshot({ ...snapshot,
    lineIntegrations: replace(snapshot.lineIntegrations, "integrationRef", { integrationRef, workspaceRef: workspace.workspaceRef, applicationRef: application.applicationRef, displayName: metadata.displayName, lineBotAccount: metadata.lineBotAccount, environment: metadata.environment, note: metadata.note || null, status: "draft", draftStatus: "saved", updatedAt: now }),
...
export function configureLineIntegration(snapshot, receipt, metadata, now = Date.now()) {
  if (receipt?.credentialStorageStatus !== "securely_stored" || !receipt?.credentialReference) throw new PlatformAccessError("CREDENTIAL_STORAGE_NOT_CONFIGURED");
  ...
  return createPlatformSnapshot({ ...snapshot,
    ...
    credentialReferences: replace(snapshot.credentialReferences, "integrationRef", { credentialReference: receipt.credentialReference, integrationRef, workspaceRef: workspace.workspaceRef, credentialStorageStatus: receipt.credentialStorageStatus, updatedAt: now }),
