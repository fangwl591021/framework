...
describe("Migration 0008 safety",()=>{
  it("rolls back permission registration and restores protection on forced failure",async()=>{
    await reset();const prior=migrations.filter((migration)=>!migration.name.includes("0008_ai_gateway"));await applyD1Migrations(env.DB,[...prior]);
