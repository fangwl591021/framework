...
describe("webhook duplicate safety",()=>{
 ...
 it("replays the same event",async()=>{const s=new LocalWebhookDeduplicator(()=>timestamp,new TestUuid());const a=await s.claim(fp());await s.complete(a.receiptId,{status:"completed"});expect(await s.claim(fp())).toMatchObject({status:"duplicate_replay",executeMutation:false,safeResult:{status:"completed"}})});
 ...
 it("expires deterministic local receipt",async()=>{const s=new LocalWebhookDeduplicator(()=>timestamp,new TestUuid(),1000);const a=await s.claim(fp());timestamp+=1001;expect((await s.claim(fp())).receiptId).not.toBe(a.receiptId)});
 it("rejects unsafe result",async()=>{const s=new LocalWebhookDeduplicator(()=>timestamp,new TestUuid());const a=await s.claim(fp());await expect(s.complete(a.receiptId,{token:"unsafe"})).rejects.toThrow("UNSAFE_WEBHOOK_RESULT")});
 it("completes idempotently",async()=>{const s=new LocalWebhookDeduplicator(()=>timestamp,new TestUuid());const a=await s.claim(fp());await s.complete(a.receiptId,{code:"OK"});await s.complete(a.receiptId,{code:"OK"});expect(s.records).toHaveLength(1)});
});
...
describe("rate and tenant isolation",()=>{
 ...
 it("releases concurrency",async()=>{const s=new LocalTenantResourceIsolation(()=>timestamp,{tenant:resource,platform:resource});await s.evaluate(ctx());s.release(ctx().tenantId);expect(s.snapshot(ctx().tenantId).concurrentRequests).toBe(0)});
});
...
describe("circuit isolation",()=>{
 ...
 it("closes on success",()=>{const s=new LocalCircuitBreaker(()=>timestamp,{failureThreshold:1,cooldownMs:1000,halfOpenProbeLimit:1});s.recordFailure(ctx({dependencyKey:"p"}));expect(s.recordSuccess(ctx({dependencyKey:"p"})).state).toBe("closed")});
 it("does not globalize tenant provider outage",async()=>{const s=new LocalCircuitBreaker(()=>timestamp,{failureThreshold:1,cooldownMs:1000,halfOpenProbeLimit:1});s.recordFailure(ctx({dependencyKey:"p"}));expect((await s.evaluate(ctx({tenantId:"019d0000-0000-7000-8000-000000000002",dependencyKey:"p"}))).admitted).toBe(true)});
});
...
describe("load shedding and backpressure",()=>{
 ...
 it.each(["/health","/ready","/status","/security/incident"])("preserves %s",route=>expect(isEmergencySafe(route)).toBe(true));
 it("uses hysteresis",()=>{const s=new LocalLoadShedding(()=>timestamp,{recoveryHysteresisMs:1000});s.activate("emergency");expect(s.recover()).toBe(false);timestamp+=1001;expect(s.recover()).toBe(true)});
